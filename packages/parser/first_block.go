...
func (p *Parser) FirstBlock() error {
	...
	err := p.CreateDltWallet(myAddress, data.Host, converter.AddressToString(myAddress), hex.EncodeToString(data.PublicKey), hex.EncodeToString(data.NodePublicKey), consts.FIRST_QDLT)
	//p.TxMaps.String["host"], myAddress, p.TxMaps.Bytes["public_key"], p.TxMaps.Bytes["node_public_key"], consts.FIRST_DLT)
	if err != nil {
		return p.ErrInfo(err)
	}
	err = p.CreateFullNode(myAddress, data.Host)
	if err != nil {
		return p.ErrInfo(err)
	}
