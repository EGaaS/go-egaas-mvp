...
func (p *NewStateParser) Main(country, currency string) (id string, err error) {
	...
	err = p.ExecSQL(`INSERT INTO "`+id+`_tables" (name, columns_and_permissions, conditions) VALUES
		...
		id+`_smart_contracts`, `{"insert": "ContractAccess(\"@0NewContract\")", "update": {"*": "ContractAccess(\"@0EditContract\", \"@0ActivateContract\")"}, "new_column": "ContractAccess(\"@0NewContractColumn\")", "general_update":  "`+sid+`"}`, psid,
		id+`_tables`, `{"insert": "ContractAccess(\"@0NewTable\")", "update": {"*": "ContractAccess(\"@0EditTable\", \"@0NewColumn\", \"@0EditColumn\")"}, "new_column": "ContractAccess(\"@0NewContractColumn\")", "general_update":  "`+sid+`"}`, psid,
